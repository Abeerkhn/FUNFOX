using AutoMapper;
using FUNFOX.NET5.Application.Exceptions;
using FUNFOX.NET5.Application.Extensions;
using FUNFOX.NET5.Application.Interfaces.Services;
using FUNFOX.NET5.Application.Interfaces.Services.Identity;
...
using FUNFOX.NET5.Infrastructure.Specifications;
using FUNFOX.NET5.Shared.Constants.Role;
...
namespace FUNFOX.NET5.Infrastructure.Services.Identity
{
    public class UserService : IUserService
    {
        ...
        private Guid GenerateCustomVerificationToken()
        {
            // Generate a random string or GUID for the verification token
        ...
        public async Task<IResult> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            var verificationToken = GenerateCustomVerificationToken();
        ...
        {
            var route = $"{origin}/api/identity/user/confirm-email?userId={user.Id}&token={user.VerificationToken}";
        ...
        public async Task<IResult<string>> ConfirmEmailAsync(string userId, string token)
        {
            ...
            // Verify if the user exists and if the provided token matches the user's verification token
            if (user == null || user.VerificationToken == null || !string.Equals(user.VerificationToken.ToString(), token, StringComparison.OrdinalIgnoreCase))
            {
                // If the user doesn't exist or the token doesn't match, return an error
                return await Result<string>.FailAsync("Invalid user ID or verification token.");
            }
            ...
            else
            {
                ...
                return await Result<string>.FailAsync("Failed to confirm email.");
            }
        }
